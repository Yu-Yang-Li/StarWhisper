...
# -*- coding: utf-8 -*-
"""
XGBoost 分类器（1121 特征集版本，使用 Optuna 超参数优化）

数据：features/train2_1121_* + test2_1121_* ；划分 data/split/1121/

输出目录：/root/shared-nvme/train_models/xgboost_optuna_1121
生成：
- 最佳模型: xgboost_optuna_1121_best.json
...
MODEL_DIR = BASE_DIR / "train_models/xgboost_optuna_1121"
...
RANDOM_STATE = 42
OPTUNA_STUDY_NAME = "xgboost_1121_optuna"
...
def objective(trial, X_train, y_train):
    ...
    params = {
        ...
        "tree_method": "auto",
        "missing": 0.0,  # 使用0.0处理缺失值（因为1121特征集中所有missing值都是0）
        "verbosity": 0,
    }
...
def plot_confusion_matrix(y_true, y_pred, classes, out_path: Path):
    ...
    plt.title("Confusion Matrix - XGBoost (Optuna Optimized, 1121)")
    plt.tight_layout()
...
def plot_feature_importance(
    ...
    plt.title(f"Feature Importance (Top {top_n}) - XGBoost (Optuna, 1121)")
    plt.tight_layout()
...
def main():
    logger.info("=" * 80)
    logger.info("XGBoost 分类器训练（Optuna 超参数优化，1121特征集）")
    logger.info("=" * 80)
    logger.info("数据: 统一划分 + features_1121")
    logger.info(f"Optuna 试验次数: {N_TRIALS}")
    logger.info(f"交叉验证折数: {N_FOLDS}")

    bundle = load_split_feature_bundle(pool="1121")
    ...
    logger.info(f"训练/验证/测试: {len(X_train)}/{len(X_val)}/{len(X_test)}")
    ...
    # 保存模型
    model_file = MODEL_DIR / "xgboost_optuna_1121_best.json"
    ...
    # 打印分类指标
    print_classification_metrics(y_test_encoded, test_pred, list(range(len(classes))))
