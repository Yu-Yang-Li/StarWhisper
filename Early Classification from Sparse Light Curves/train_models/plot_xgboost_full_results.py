...
# -*- coding: utf-8 -*-
"""
为已训练的 XGBoost Optuna 1117 模型生成完整可视化

结构对齐 plot_xgboost_reduced_results.py
数据：原有 train2/test2 1117 特征 CSV + data/split/varlen/ 中 15% 测试集索引
"""
...
POOL = "varlen"
MODEL_DIR = BASE_DIR / "train_models/xgboost_full"
RESULTS_DIR = MODEL_DIR / "results"
MODEL_FILE = MODEL_DIR / "xgboost_full_best.json"
...
def plot_confusion_matrix(y_true, y_pred, classes, out_path: Path) -> None:
    ...
    plt.title("Confusion Matrix (1117, 15% test split)")
    plt.tight_layout()
...
def plot_feature_importance(
    ...
    plt.title(f"Feature Importance Top {top_n} (1117)")
    plt.tight_layout()
...
def plot_accuracy_by_band(df: pd.DataFrame, out_csv: Path, out_png: Path) -> None:
    ...
    plt.ylabel("Accuracy")
    plt.title("Accuracy by Band Code (1117)")
...
def plot_accuracy_by_numpoints(df: pd.DataFrame, out_csv: Path, out_png: Path) -> None:
    ...
    plt.ylabel("Accuracy")
    plt.title("Accuracy by Number of Points (1117)")
    plt.grid(alpha=0.3)
    plt.ylim(0.5, 1.0)
    plt.tight_layout()
    plt.savefig(out_png, dpi=300, bbox_inches="tight")
...
def main() -> None:
    logger.info("=" * 80)
    logger.info("XGBoost Optuna 1117 结果可视化（统一 75/10/15 划分）")
