...
- 特征文件（不变）: train2_1117_balanced / test2_1117_balanced
- 划分: data/split/varlen/（75% 训 / 10% 验证早停 / 15% 测试仅最终评估）
...
MODEL_DIR = BASE_DIR / "train_models/transformer_classifier_model_1117"
...
def save_config(bundle: DataBundle):
    ...
    lines = [
        "Transformer (1117) 训练配置\n",
        "DATA: data/split + features_1117\n",
        f"N_EPOCHS: {N_EPOCHS}\n",
