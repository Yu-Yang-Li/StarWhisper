...
OUT_DIR = Path(__file__).resolve().parent / "results" / "feature_ablation"
...
def main():
    ...
    r = run_experiment(X_tr, y_tr, X_te, y_te, all_cols, "ablation_all_57_features")
    results.append(r)
    ...
    r = run_experiment(X_tr[:, no_ls_mask], y_tr, X_te[:, no_ls_mask], y_te,
                       no_ls_cols, "ablation_39_features_no_ls")
    results.append(r)
    ...
    r = run_experiment(X_tr[:, top15_mask], y_tr, X_te[:, top15_mask], y_te,
                       top15_cols, "ablation_top15_by_importance")
    results.append(r)
    ...
    r = run_experiment(X_tr[:, combined_41_mask], y_tr, X_te[:, combined_41_mask], y_te,
                       combined_41_cols, "ablation_top15_plus_39_non_ls")
    results.append(r)
    ...
    if ls_mask.sum() > 0:
        r = run_experiment(X_tr[:, ls_mask], y_tr, X_te[:, ls_mask], y_te,
                           ls_cols, "ablation_ls_features_only")
        results.append(r)
    ...
    if miss_mask.sum() > 0:
        r = run_experiment(X_tr[:, miss_mask], y_tr, X_te[:, miss_mask], y_te,
                           miss_cols, "ablation_missing_indicators_only")
        results.append(r)

    # Exp 7: 39 non-LS features + 2 best LS features
    ...
    r = run_experiment(X_tr[:, combo_41_mask], y_tr, X_te[:, combo_41_mask], y_te,
                       combo_41_cols, "ablation_39_plus_2_ls")
    results.append(r)
    ...
    if periodic_mask.sum() > 0:
        r = run_experiment(X_tr[:, periodic_mask], y_tr, X_te[:, periodic_mask], y_te,
                           periodic_cols, "ablation_is_periodic_only")
        results.append(r)
