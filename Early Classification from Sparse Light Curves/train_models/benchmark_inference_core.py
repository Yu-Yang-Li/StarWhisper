...
用法（推荐）:
  ...
  conda run -n astro_classifier python train_models/run_benchmark_inference.py --device both

  # 跳过旧路径 legacy 模型
  conda run -n astro_classifier python train_models/run_benchmark_inference.py --skip-legacy
...
def _load_feature_transformer(model_dir: Path, device: torch.device) -> tuple:
    from finetune_transformer_feat_varlen import TransformerClassifier  # noqa: WPS433
...
def benchmark_e2e_small_50obs(device: torch.device, max_samples: Optional[int]) -> dict:
    ...
    model_dir = TRAIN_MODELS / "e2e_transformer_50obs_pretrain"
...
def estimate_feature_times(
    ...
    import run_benchmark_inference as leg
...
def run_benchmarks(
    ...
    jobs: list[tuple[str, Callable[[Optional[int]], dict], str]] = [
        (
            "xgb_1117",
            lambda n: benchmark_xgboost(TRAIN_MODELS / "xgboost_full", "varlen", n),
            "cpu",
        ),
        (
            "xgb_1121",
            lambda n: benchmark_xgboost(TRAIN_MODELS / "xgboost_reduced", "1121", n),
            "cpu",
        ),
        (
            "tf_feat_50obs",
    ...
    def e2e_matched_50(n: Optional[int]):
        ...
        if n:
            X, y = X[:n], y[:n]
        loader = DataLoader(NumpyTensorDataset(X, y), batch_size=BATCH_SIZE, shuffle=False)
        model_dir = TRAIN_MODELS / "e2e_transformer_50obs_matched_pretrain"
    ...
    def rnn(n: Optional[int]):
        from train_lstm_varlen import LSTMClassifier
