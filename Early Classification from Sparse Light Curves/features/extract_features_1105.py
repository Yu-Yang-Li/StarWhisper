...
# -*- coding: utf-8 -*-
"""
ZTF光变曲线特征提取脚本（1117版 - 在1105版基础上新增缺失指示特征，并加入SN目录）
...
- 训练集新增 /root/shared-nvme/SN 目录，覆盖超新星数据
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("extract_features_1117.log", encoding="utf-8"),
...
def calculate_lomb_scargle_features_sn(time, mag):
    """SN版本的Lomb-Scargle特征计算（参考extract_features_sn_1105.py）"""
    if len(time) < 5:
        return {
...
def extract_sn_features_from_file(csv_file: Path) -> Optional[dict]:
    """从SN文件提取特征（完全按照extract_features_sn_1105.py，但加上is_missing特征）"""
    try:
        df = pd.read_csv(csv_file)
