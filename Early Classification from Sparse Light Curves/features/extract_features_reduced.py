...
# -*- coding: utf-8 -*-
"""
ZTF光变曲线特征提取脚本（精简 39 特征 · 无 Lomb-Scargle / is_missing）

在完整 57 特征集基础上移除所有 Lomb-Scargle 相关特征和 is_missing 特征。

移除特征：
- 所有ls_开头的频域特征（ls_power_to_median、ls_entropy、ls_harmonic_ratio等）
- 所有ls_相关的缺失指示特征
- is_periodic特征（依赖ls_power_to_median）
- 所有is_missing特征（skewness_is_missing、kurtosis_is_missing等）

输出文件名包含 1121 标识：
train2_1121_YYYYmmdd_HHMMSS.csv / test2_1121_YYYYmmdd_HHMMSS.csv
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("extract_features_reduced.log", encoding="utf-8"),
...
def main():
    logger.info("=" * 80)
    logger.info("【ZTF 特征提取 - 1121 版（移除lombscagle特征）】")
    logger.info("目标：适用于 3~30 点稀疏数据，移除所有lombscagle相关特征")
    ...
    # 步骤1: 处理train2数据
    logger.info("\n" + "=" * 80)
    logger.info("步骤1: 处理train2数据")
    logger.info("=" * 80)
    train2_features = []
    for cat in CATEGORIES:
        feats = process_category(cat)
        train2_features.extend(feats)

    if not train2_features:
        logger.error("train2数据未提取到任何有效特征")
        return

    train2_df = pd.DataFrame(train2_features)
    logger.info(f"train2数据提取完成: {len(train2_df)} 个样本")

    # 步骤2: 处理SN数据（不参与平衡）
    logger.info("\n" + "=" * 80)
    logger.info("步骤2: 处理SN数据（不参与平衡）")
    logger.info("=" * 80)
    ...
    # 步骤3: 合并train2和SN数据
    logger.info("\n" + "=" * 80)
    logger.info("步骤3: 合并train2和SN数据")
    logger.info("=" * 80)
    if len(sn_df) > 0:
        df = pd.concat([train2_df, sn_df], ignore_index=True)
        logger.info(
            f"合并后总样本数: {len(df)} (train2: {len(train2_df)}, SN: {len(sn_df)})"
        )
    else:
        df = train2_df
        logger.info(f"总样本数: {len(df)} (仅train2)")

    # 列顺序（移除所有ls_开头的特征和is_periodic）
    feature_order = [
        "file_path",
        "num_points",
        "mean_mag",
    ...
    # 步骤5: 对train2类别进行4倍平衡（SN不参与）
    logger.info("\n" + "=" * 80)
    logger.info("步骤5: 平衡数据集（train2类别4倍平衡，SN不参与）")
    logger.info("=" * 80)
    ...
    # 保存未平衡版本
    train_out = output_features_dir / f"train4_1121_{timestamp}.csv"
    test_out = output_features_dir / f"test4_1121_{timestamp}.csv"
    ...
    # 保存平衡版本
    train_balanced_out = output_features_dir / f"train2_1121_{timestamp}_balanced.csv"
    test_balanced_out = output_features_dir / f"test2_1121_{timestamp}_balanced.csv"
