...
# -*- coding: utf-8 -*-
"""
ZTF光变曲线特征提取脚本（完整 57 特征 · train2/varlen 版）

57 个手工特征，从 `train2/` 目录提取（varlen 池）。
当前推荐版本为上级目录的 `extract_features_full.py`（从 `train4/` 提取）。

输出文件名包含 1117 标识：
train2_1117_YYYYmmdd_HHMMSS.csv / test2_1117_YYYYmmdd_HHMMSS.csv
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("extract_features_full_varlen.log", encoding="utf-8"),
...
def extract_sn_features_from_file(csv_file: Path) -> Optional[dict]:
    """从SN文件提取特征（与 extract_features_full_supernova.py 一致，并加上 is_missing 特征）"""
    try:
        df = pd.read_csv(csv_file)
...
def main():
    logger.info("=" * 80)
    logger.info("【ZTF 特征提取 - 1117 扩展版】")
    logger.info("目标：适用于 3~30 点稀疏数据，新增多项鲁棒特征")
    ...
    # 步骤1: 处理train2数据（57个特征）
    logger.info("\n" + "=" * 80)
    logger.info("步骤1: 处理train2数据（提取57个特征）")
    logger.info("=" * 80)
    train2_features = []
    ...
    # 保存未平衡版本
    train_out = output_features_dir / f"train2_1117_{timestamp}.csv"
    test_out = output_features_dir / f"test2_1117_{timestamp}.csv"
    ...
    # 保存平衡版本
    train_balanced_out = output_features_dir / f"train2_1117_{timestamp}_balanced.csv"
    test_balanced_out = output_features_dir / f"test2_1117_{timestamp}_balanced.csv"
