...
# -*- coding: utf-8 -*-
"""
SN 数据特征提取脚本（完整 57 特征 · 仅超新星）

针对 SN 目录中的 ZTF / ATLAS 采样数据，使用与 `extract_features_full_varlen.py`
相同的 57 特征提取方法。
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            BASE_DIR / "features/extract_features_full_supernova.log",
            encoding="utf-8",
        ),
...
def main():
    logger.info("=" * 80)
    logger.info("【SN 特征提取 - 1117 特征集】")
    logger.info("目标：适用于 SN 采样数据（3~30 点），使用 1117 完整特征集")
