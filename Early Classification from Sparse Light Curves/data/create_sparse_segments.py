...
# -*- coding: utf-8 -*-
"""
从原始 ZTF 光变 CSV 切分稀疏片段，写入 train4/（50 点）或 train2/（3–30 点）。

输入：source_dir 下按原始类别命名的子文件夹，每个 CSV 含 filtercode / mjd / mag。
输出：output_dir/<raw_category>/*.csv，mjd 归零，文件名编码长度与波段。

示例：
  # train4：50 点固定长度（E2E 预训练池）
  python data/create_sparse_segments.py --pool 50obs

  # train2：3–30 点变长（varlen 主实验池）
  python data/create_sparse_segments.py --pool varlen

  # 指定原始数据根目录
  python data/create_sparse_segments.py --source-dir /path/to/raw_ztf --pool 50obs
"""
