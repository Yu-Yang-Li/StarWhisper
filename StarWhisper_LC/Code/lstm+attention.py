...
class Attention(nn.Module):
    def __init__(self, activation=None):
        ...
        self.W = nn.Linear(self.in_dim, 1, bias=False)
...
class LCModel(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.sum(x * attention_weight, dim=1)
        x = F.relu(self.fc1(x))
        x = self.dropout(x)
        x = self.fc2(x)
...
torch.save(best_model.state_dict(), 'labest_model.pth')
#
# # %%
